...
def model_fn(actions,N=2,filters=24):
    ...
    #normal cell
    for i in range(N):
        x=build_cell(ip1,ip2, filters, actions, B,'block_'+str(block_index),stride1,stride2)
        ip1=ip2
        ip2=x
        block_index+=1
...
def parse_action(ip, filters, action,input_name, strides=(1, 1)):
    ...
    if action == '3x3 dconv':
        x = SeparableConv2D(filters, (3, 3), strides=strides, padding='same',name=input_name+'_1')(ip)
    ...
    if action == '5x5 dconv':
        x = SeparableConv2D(filters, (5, 5), strides=strides, padding='same',name=input_name+'2')(ip)
        x = BatchNormalization()(x)
        x = Activation('relu')(x)
        return x
    ...
    if action == '7x7 dconv':
        x = SeparableConv2D(filters, (7, 7), strides=strides, padding='same',name=input_name+'3')(ip)
        x = BatchNormalization()(x)
        x = Activation('relu')(x)
        return x
    ...
    if action == '1x7-7x1 conv':
        x = Conv2D(filters, (1, 7), strides=strides, padding='same',name=input_name+'4')(ip)
        x = BatchNormalization()(x)
        x = Activation('relu')(x)
        x = Conv2D(filters, (7, 1), strides=(1, 1), padding='same',name=input_name+'5')(x)
        x = BatchNormalization()(x)
        x = Activation('relu')(x)
        return x
    ...
    if action == '3x3 conv':
        x = Conv2D(filters, (3, 3), strides=strides, padding='same',name=input_name+'6')(ip)
        x = BatchNormalization()(x)
        x = Activation('relu')(x)
        return x
    ...
    if action == '3x3 maxpool':
        return MaxPool2D((3, 3), strides=strides, padding='same',name=input_name+'7')(ip)

    # applies a 3x3 avgpool
    if action == '3x3 avgpool':
        return AveragePooling2D((3, 3), strides=strides, padding='same',name=input_name+'8')(ip)
    ...
    if strides == (2, 2):
        ...
        x = Conv2D(input_filters, (1, 1), strides=strides, padding='same',name=input_name+'9')(ip)
        x = BatchNormalization()(x)
        x = Activation('relu')(x)
        return x
    else:
...
def build_cell(ip, filters, actions, B, stride):
    ...
    return op
"""
def build_cell(ip1,ip2, filters, actions, B,name, stride1=(1,1),stride2=(1,1)):
    # if cell size is 1 block only
    inputs=[ip1,ip2]
    ...
    if B == 1:
        index=actions[0]+1
        left = parse_action(inputs[index], filters, actions[1], strides=stride[index],input_name=name+'left')
        index=actions[2]+1
        right = parse_action(inputs[index], filters, actions[3], strides=stride[index],input_name=name+'right')
        return concatenate([left, right], axis=-1)
    ...
    for i in range(B):
        index=actions[i*4]+1
        left_action = parse_action(inputs[index], filters, actions[i * 4+1], strides=stride[index],input_name=name+'left')
        index=actions[i*4+2]+1
        right_action = parse_action(inputs[index], filters, actions[i * 4+3], strides=stride[index],input_name=name+'right')
        action = concatenate([left_action, right_action], axis=-1)
        actions.append(action)
